...
func TestEventGRPC_GetFavorites(t *testing.T) {
	t.Parallel()
	...
	}{
		{
			...
			setupFunc: func(ctrl *gomock.Controller) *event.ServerAPI {
				...
				mockEventGetter.EXPECT().
					GetFavorites(context.Background(), 1, models.PaginationParams{Limit: 1, Offset: 0}).
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := tt.setupFunc(ctrl).GetFavorites(context.Background(), tt.req)
