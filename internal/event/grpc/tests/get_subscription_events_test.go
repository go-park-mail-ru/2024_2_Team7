...
func TestEventGRPC_GetSubsciptionEvents(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{
			name: "success get subscriprtion events",
			req: &pb.GetSubscriptionsRequest{
				ID: 1,
			...
			setupFunc: func(ctrl *gomock.Controller) *event.ServerAPI {
				...
				mockEventGetter.EXPECT().
					GetSubscriptionEvents(context.Background(), 1, models.PaginationParams{Limit: 1, Offset: 0}).
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := tt.setupFunc(ctrl).GetSubscriptionsEvents(context.Background(), tt.req)
