...
func TestEventGRPC_GetEventByID(t *testing.T) {
	t.Parallel()
	...
	}{
		{
			...
			setupFunc: func(ctrl *gomock.Controller) *event.ServerAPI {
				...
				mockEventGetter.EXPECT().
					GetEventByID(context.Background(), 1).
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := tt.setupFunc(ctrl).GetEventByID(context.Background(), tt.req)
