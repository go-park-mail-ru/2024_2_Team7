...
func TestEventGRPC_GetUpcomingEvents(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{
			name: "success get upcoming events",
			req: &pb.PaginationParams{
				Limit:  1,
				Offset: 0,
			},
			setupFunc: func(ctrl *gomock.Controller) *event.ServerAPI {
				...
				mockEventGetter.EXPECT().
					GetUpcomingEvents(context.Background(), models.PaginationParams{Limit: 1, Offset: 0}).
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := tt.setupFunc(ctrl).GetUpcomingEvents(context.Background(), tt.req)
