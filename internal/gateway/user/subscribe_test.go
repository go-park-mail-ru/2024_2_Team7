...
func TestUserHandler_Subscribe(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			setupFunc: func(ctrl *gomock.Controller) *UserHandlers {
				...
				serviceMock.EXPECT().Subscribe(gomock.Any(), subscriptionRequest).Return(nil, nil)
		...
		},
		{
			...
			setupFunc: func(ctrl *gomock.Controller) *UserHandlers {
				serviceMock := mocks.NewMockUserServiceClient(ctrl)
				serviceMock.EXPECT().Subscribe(gomock.Any(), subscriptionRequest).Return(nil, status.Error(codes.NotFound, grpc.ErrUserNotFound))
		...
		},
		{
			...
			setupFunc: func(ctrl *gomock.Controller) *UserHandlers {
				serviceMock := mocks.NewMockUserServiceClient(ctrl)
				serviceMock.EXPECT().Subscribe(gomock.Any(), subscriptionRequest).Return(nil, status.Error(codes.AlreadyExists, grpc.ErrSubscriptionAlreadyExists))
		...
		},
		{
			...
			setupFunc: func(ctrl *gomock.Controller) *UserHandlers {
				serviceMock := mocks.NewMockUserServiceClient(ctrl)
				serviceMock.EXPECT().Subscribe(gomock.Any(), subscriptionRequest).Return(nil, status.Error(codes.Internal, grpc.ErrInternal))
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			tt.setupFunc(ctrl).Subscribe(recorder, tt.req)
